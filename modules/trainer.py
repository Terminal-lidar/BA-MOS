...
from modules.BAMOS import BAMOS
...
class bce2d(torch.nn.Module):
    def __init__(self):
        super(bce2d, self).__init__()
        pass
    def forward(self, input, target):
        # n, c, h, w = input.size() # b, 8, h, w
        self.log_p = input.transpose(1, 2).transpose(2, 3).contiguous().view(1, -1) # 展成长向量
        self.target_t = target.transpose(1, 2).transpose(2, 3).contiguous().view(1, -1)
        self.target_t = self.target_t.to('cuda') # 
        target_trans = self.target_t.clone()
        pos_index = (self.target_t == 1)
        neg_index = (self.target_t == 0)
        ignore_index = (self.target_t > 1)
        ...
        weight = torch.Tensor(self.log_p.size()).fill_(0)
        weight = weight.numpy()
        ...
        loss = F.binary_cross_entropy_with_logits(self.log_p, self.target_t, self.weight, reduction='mean')
        return loss
...
class Trainer():
    def __init__(self, ARCH, DATA, datadir, logdir, path=None, point_refine=False, local_rank=0):
        ...
        self.batch_time_e = AverageMeter() # 验证时间
        ...
        with torch.no_grad():
            self.model = BAMOS(nclasses=self.parser.get_n_classes(),
    ...
    def set_loss_function(self, point_refine):
        ...
        """
        self.criterion = nn.NLLLoss(weight=self.loss_w.double()).cuda() # double() -> torch.float64
        self.movable_criterion = nn.NLLLoss(weight=self.movable_loss_w.double()).cuda()
        self.bce2d = bce2d().cuda()
        if not point_refine:
    ...
    def load_pretrained_model(self):
        ...
        if not self.point_refine:
            ...
            print("load the pretrained model of BAMOS")
        else:
            checkpoint = "BAMOS_valid_best"
            w_dict = torch.load(f"{self.path}/{checkpoint}", map_location=lambda storage, loc: storage)
            # self.model.load_state_dict(w_dict['state_dict'], strict=True)
            self.model.load_state_dict({k.replace('module.',''):v for k,v in w_dict['state_dict'].items()})
            self.optimizer.load_state_dict(w_dict['optimizer'])
            print("load the coarse model of BAMOS_valid_best")
    ...
    def train(self):
        ...
        for epoch in range(self.epoch, self.ARCH["train"]["max_epochs"]):
            self.parser.train_sampler.set_epoch(epoch) # 实验室单卡跑要注释，在每个 epoch 开始时更新或调整数据加载器的状态，以确保在分布式训练中数据能够被正确地打乱和分配给各个 worker。
            # train for 1 epoch
    ...
    def train_epoch(self, train_loader, model, all_criterion, optimizer,
        ...
        for i, (in_vol, proj_mask, all_proj_labels, _, path_seq, path_name,
            ...
            proj_labels, movable_proj_labels, edgemap = all_proj_labels
            ...
            output, _, movable_output, _ , edg = model(in_vol)

            # loss_m = criterion(torch.log(output.clamp(min=1e-8)), proj_labels) + self.ls(output, proj_labels.long())
            moving_loss_m = criterion(torch.log(output.clamp(min=1e-8)).double(), proj_labels).float() + self.ls(output, proj_labels.long()) + self.bce2d(edg, edgemap)
            ...
            # their norms 计算每个参数在当前优化步骤中的更新情况
            update_ratios = []
            ...
            if i % self.ARCH["train"]["report_batch"] == 0 and self.local_rank == 0:
                ...
                print(str_line)
                save_to_txtlog(self.logdir, 'log.txt', str_line) # 写日志
    ...
    def validate(self, val_loader, model, all_criterion, all_evaluator, class_func, color_fn, save_scans=False):
        ...
        with torch.no_grad():
            ...
            for i, (in_vol, proj_mask, all_proj_labels, _, path_seq, path_name,
                ...
                bcd = self.bce2d(edg, edgemap)
                moving_loss = wce + jacc + bcd
