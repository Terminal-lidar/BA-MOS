...
class TrainerRefine(Trainer):
    ...
    def train_epoch(self, train_loader, model, criterion, optimizer,
        ...
        for i, (in_vol, proj_mask, all_proj_labels, unproj_labels, path_seq, path_name,
            ...
            proj_labels, movable_proj_labels, _ = all_proj_labels
            ...
            if epoch <= start_epoch:
                with torch.no_grad():
                    output, last_feature, movable_output, movalbe_last_feature, _ = model(in_vol)
            else:
                output, last_feature, movable_output, movalbe_last_feature, _ = model(in_vol)
    ...
    def validate(self, val_loader, model, criterion, evaluator, class_func, color_fn, save_scans=False):
        ...
        with torch.no_grad():
            ...
            for i, (in_vol, proj_mask, all_proj_labels, unproj_labels, path_seq, path_name,
                ...
                # output = model(in_vol)
                with torch.no_grad():
                    output, last_feature, movable_output, movable_last_feature, _ = model(in_vol)
