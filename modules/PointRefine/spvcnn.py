...
class FeatureExtractBlock(nn.Module):
    def __init__(self, inc, outc, stride=1, dilation=1):
        ...
        self.conv5 = nn.Sequential(
            spnn.Conv3d(inc, outc, kernel_size=5, dilation=dilation, stride=stride),
            spnn.BatchNorm(outc),
            spnn.ReLU(True)
        )
        self.conv_cat = nn.Sequential(
            spnn.Conv3d(outc * 3, outc, kernel_size=1, dilation=dilation, stride=stride),
            spnn.BatchNorm(outc),
            spnn.ReLU(True),
            spnn.Conv3d(outc, outc, kernel_size=3, dilation=dilation, stride=stride),
            spnn.BatchNorm(outc),
        )
    ...
    def forward(self, x):
        out = self.relu(self.conv_cat(torchsparse.cat([self.conv1(x), self.conv3(x), self.conv5(x)])) + self.downsample(x))
        channel_attn_score = self.act(self.channel_attn(out).F)
        out.F = out.F + out.F * channel_attn_score # 
        return out
...
class SPVCNN(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.stage1 = nn.Sequential(
            ...
            FeatureExtractBlock(cs[1], cs[1], stride=1, dilation=1),
        )

        self.stage2 = nn.Sequential(
            BasicConvolutionBlock(cs[1], cs[1], ks=2, stride=2, dilation=1),
            ResidualBlock(cs[1], cs[2], ks=3, stride=1, dilation=1),
            FeatureExtractBlock(cs[2], cs[2], stride=1, dilation=1),
        )

        self.stage3 = nn.Sequential(
            BasicConvolutionBlock(cs[2], cs[2], ks=2, stride=2, dilation=1),
            ResidualBlock(cs[2], cs[3], ks=3, stride=1, dilation=1),
            ResidualBlock(cs[3], cs[3], ks=1, stride=1, dilation=1),
        )

        self.stage4 = nn.Sequential(
            BasicConvolutionBlock(cs[3], cs[3], ks=2, stride=2, dilation=1),
            ResidualBlock(cs[3], cs[4], ks=3, stride=1, dilation=1),
            ResidualBlock(cs[4], cs[4], ks=1, stride=1, dilation=1),
        )
        ...
        self.up1 = nn.ModuleList([
            ...
            nn.Sequential(
                ...
                ResidualBlock(cs[5], cs[5], ks=1, stride=1, dilation=1),
            )
        ])
        ...
        self.up2 = nn.ModuleList([
            ...
            nn.Sequential(
                ...
                ResidualBlock(cs[6], cs[6], ks=1, stride=1, dilation=1),
            )
        ])
        ...
        self.up3 = nn.ModuleList([
            ...
            nn.Sequential(
                ...
                FeatureExtractBlock(cs[7], cs[7], stride=1, dilation=1),
            )
        ])
        ...
        self.up4 = nn.ModuleList([
            ...
            nn.Sequential(
                ...
                FeatureExtractBlock(cs[8], cs[8], stride=1, dilation=1),
            )
        ])
    ...
    def forward(self, x):
        ...
        x1 = self.stage1(x1)    # x1 (95150, 32)
        x2 = self.stage2(x1)    # x2 (69207, 64)
        x3 = self.stage3(x2)    # x3 (16218, 128)
        x4 = self.stage4(x3)    # x4 (2197, 256)
        z1 = voxel_to_point(x4, z0)     # z1 (99349, 256)
        z1.F = z1.F + self.point_transforms[0](z0.F)

        y1 = point_to_voxel(x4, z1)     # y1 (2197, 256)
        y1.F = self.dropout(y1.F)
        y1 = self.up1[0](y1)            # y1 (2197, 256)  ->  (16205, 256)
        y1 = torchsparse.cat([y1, x3])
        y1 = self.up1[1](y1)

        y2 = self.up2[0](y1)        # y1 (16205, 256)  ->  (69181, 128)
        y2 = torchsparse.cat([y2, x2])
        y2 = self.up2[1](y2)
        z2 = voxel_to_point(y2, z1) # y2 (19181, 128)  ->  z2 (99348, 128)
...
if __name__ == "__main__":
    ...
    pytorch_total_params = sum(p.numel() for p in PointHead.parameters() if p.requires_grad)
    print(f"Total Params: {pytorch_total_params}")
    
    flop_analyzer = FlopCountAnalysis(PointHead, sparse_inputs)
    print(f"FLOPs: {flop_analyzer.total() / 1e9:.4f} GFLOPs")
